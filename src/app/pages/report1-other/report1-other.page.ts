...
@Component({
  selector: 'app-report1-other',
  templateUrl: './report1-other.page.html',
  styleUrls: ['./report1-other.page.scss'],
})
export class Report1OtherPage implements OnInit {
  itemList;
  sortSupplier= [];
  ...
  ngOnInit() {
    if(this.itemList){
      this.itemList.forEach(element => {
        console.log(element)
        element.other.forEach(elementMat => {
          ...
          if(index === -1){
            ...
            var matindex =  this.sortSupplier[index].others.findIndex(x => x.id == elementMat.id)
            if(matindex === -1){
              this.sortSupplier[index].others.push({
                name : elementMat.name,
                qty :elementMat.qty*element.qty,
                price : elementMat.rate,
                unit : elementMat.unit,
              ...
              this.sortSupplier[index].others[matindex].qty += (elementMat.qty*element.qty)
            }
          }
        });
      });
