...
import { AddboqitemG2Page } from '../addboqitem-g2/addboqitem-g2.page';

@Component({
  selector: 'app-boqitem-g2',
  templateUrl: './boqitem-g2.page.html',
  styleUrls: ['./boqitem-g2.page.scss'],
})
export class BoqitemG2Page implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalboqitems2`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalboqitems2/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    this.afs.doc(`boq/boq/generalboqitems2/${e.data.id}`).update({
      code : e.data.code,
      name : e.data.name,
  ...
  async delall(){
    ...
    this.afs.collection(`boq/boq/generalboqitems2`).get().subscribe(value=>{

      value.docs.forEach(doc =>{
        this.afs.doc(`boq/boq/generalboqitems2/${doc.ref.id}`).delete()
      })
