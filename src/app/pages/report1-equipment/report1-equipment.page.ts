...
@Component({
  selector: 'app-report1-equipment',
  templateUrl: './report1-equipment.page.html',
  styleUrls: ['./report1-equipment.page.scss'],
})
export class Report1EquipmentPage implements OnInit {
  itemList;
  sortSupplier= [];
  ...
  ngOnInit() {
    if(this.itemList){
      this.itemList.forEach(element => {
        console.log(element)
        element.equipment.forEach(elementMat => {
          ...
          if(index === -1){
            ...
            var matindex =  this.sortSupplier[index].equipments.findIndex(x => x.id == elementMat.id)
            if(matindex === -1){
              this.sortSupplier[index].equipments.push({
                name : elementMat.name,
                qty :elementMat.qty*element.qty,
                price : elementMat.rate,
                unit : elementMat.unit,
              ...
              this.sortSupplier[index].equipments[matindex].qty += (elementMat.qty*element.qty)
            }
          }
        });
      });
