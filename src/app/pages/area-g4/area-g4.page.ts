...
import { AddareaG4Page } from '../addarea-g4/addarea-g4.page';

@Component({
  selector: 'app-area-g4',
  templateUrl: './area-g4.page.html',
  styleUrls: ['./area-g4.page.scss'],
})
export class AreaG4Page implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalareas4`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalareas4/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    this.afs.doc(`boq/boq/generalareas4/${e.data.id}`).update({
      code : e.data.code,
      name : e.data.name,
    })
  }
  async delall(){
    ...
    this.afs.collection(`boq/boq/generalareas4`).get().subscribe(value=>{

      value.docs.forEach(doc =>{
        this.afs.doc(`boq/boq/generalareas4/${doc.ref.id}`).delete()
      })
