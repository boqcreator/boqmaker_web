...
import { AddareaG3Page } from '../addarea-g3/addarea-g3.page';

@Component({
  selector: 'app-area-g3',
  templateUrl: './area-g3.page.html',
  styleUrls: ['./area-g3.page.scss'],
})
export class AreaG3Page implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalareas3`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalareas3/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    this.afs.doc(`boq/boq/generalareas3/${e.data.id}`).update({
      code : e.data.code,
      name : e.data.name,
    })
  }
  async delall(){
    ...
    this.afs.collection(`boq/boq/generalareas3`).get().subscribe(value=>{

      value.docs.forEach(doc =>{
        this.afs.doc(`boq/boq/generalareas3/${doc.ref.id}`).delete()
      })
