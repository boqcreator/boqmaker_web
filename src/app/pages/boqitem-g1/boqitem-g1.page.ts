...
import { AddboqitemG1Page } from '../addboqitem-g1/addboqitem-g1.page';

@Component({
  selector: 'app-boqitem-g1',
  templateUrl: './boqitem-g1.page.html',
  styleUrls: ['./boqitem-g1.page.scss'],
})
export class BoqitemG1Page implements OnInit {
    ...
    this.rowData =  this.afs.collection(`boq/boq/generalboqitems1`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()
  
    }
  
    Del(item){
      this.afs.doc(`boq/boq/generalboqitems1/${item.rowData.id}`).delete().then(()=>{
        alert("Deleted")
      })
    ...
    onCellValueChanged(e){
      this.afs.doc(`boq/boq/generalboqitems1/${e.data.id}`).update({
        code : e.data.code,
        name : e.data.name,
        unit : e.data.unit,
    ...
    async delall(){
      ...
      this.afs.collection(`boq/boq/generalboqitems1`).get().subscribe(value=>{
  
        value.docs.forEach(doc =>{
          this.afs.doc(`boq/boq/generalboqitems1/${doc.ref.id}`).delete()
        })
