...
@Component({
  selector: 'app-addboqitem-g',
  templateUrl: './addboqitem-g.page.html',
  styleUrls: ['./addboqitem-g.page.scss'],
})
export class AddboqitemGPage implements OnInit {
  arrayBuffer:any;
  file:File;
  data: [][];
  datatable = [];
  ...
  async additem(){
    ...
    this.afs.doc(`boq/boq/generalboqitems/${this.id}`).set({
      id: this.id,
      name : this.name,
      code : this.code,
      unit : this.unit,
