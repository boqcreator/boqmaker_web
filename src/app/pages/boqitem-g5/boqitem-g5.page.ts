...
import { AddboqitemG5Page } from '../addboqitem-g5/addboqitem-g5.page';

@Component({
  selector: 'app-boqitem-g5',
  templateUrl: './boqitem-g5.page.html',
  styleUrls: ['./boqitem-g5.page.scss'],
})
export class BoqitemG5Page implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalboqitems5`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalboqitems5/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    this.afs.doc(`boq/boq/generalboqitems5/${e.data.id}`).update({
      code : e.data.code,
      name : e.data.name,
  ...
  async delall(){
    ...
    this.afs.collection(`boq/boq/generalboqitems5`).get().subscribe(value=>{

      value.docs.forEach(doc =>{
        this.afs.doc(`boq/boq/generalboqitems5/${doc.ref.id}`).delete()
      })
