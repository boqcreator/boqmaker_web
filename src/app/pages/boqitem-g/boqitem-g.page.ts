...
export class BoqitemGPage implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalboqitems`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalboqitems/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    this.afs.doc(`boq/boq/generalboqitems/${e.data.id}`).update({
      code : e.data.code,
      name : e.data.name,
