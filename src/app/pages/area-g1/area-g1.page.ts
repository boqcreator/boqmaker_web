...
import { AddareaG1Page } from '../addarea-g1/addarea-g1.page';

@Component({
  selector: 'app-area-g1',
  templateUrl: './area-g1.page.html',
  styleUrls: ['./area-g1.page.scss'],
})
export class AreaG1Page implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalareas1`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalareas1/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    if(e.data.code && e.data.name){
      this.afs.doc(`boq/boq/generalareas1/${e.data.id}`).update({
        code : e.data.code,
        name : e.data.name,
      })
    }
  ...
  async delall(){
    ...
    this.afs.collection(`boq/boq/generalareas1`).get().subscribe(value=>{

      value.docs.forEach(doc =>{
        this.afs.doc(`boq/boq/generalareas1/${doc.ref.id}`).delete()
      })
