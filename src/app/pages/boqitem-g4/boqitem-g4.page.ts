...
import { AddboqitemG4Page } from '../addboqitem-g4/addboqitem-g4.page';

@Component({
  selector: 'app-boqitem-g4',
  templateUrl: './boqitem-g4.page.html',
  styleUrls: ['./boqitem-g4.page.scss'],
})
export class BoqitemG4Page implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalboqitems4`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalboqitems4/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    this.afs.doc(`boq/boq/generalboqitems4/${e.data.id}`).update({
      code : e.data.code,
      name : e.data.name,
  ...
  async delall(){
    ...
    this.afs.collection(`boq/boq/generalboqitems4`).get().subscribe(value=>{

      value.docs.forEach(doc =>{
        this.afs.doc(`boq/boq/generalboqitems4/${doc.ref.id}`).delete()
      })
