...
import { AddmaterialGPage } from '../addmaterial-g/addmaterial-g.page';

@Component({
  selector: 'app-material-g',
  templateUrl: './material-g.page.html',
  styleUrls: ['./material-g.page.scss'],
})
export class MaterialGPage implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalmaterials`).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalmaterials/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    this.afs.doc(`boq/boq/generalmaterials/${e.data.id}`).update({
      name : e.data.name,
      des : e.data.des,
      price : e.data.price,
      unit : e.data.unit
  ...
  async delall(){
    ...
    this.afs.collection(`boq/boq/generalmaterials`).get().subscribe(value=>{
      ...
      value.docs.forEach(doc =>{
        this.afs.doc(`boq/boq/generalmaterials/${doc.ref.id}`).delete()
      })
