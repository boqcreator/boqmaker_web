...
export class AreaGPage implements OnInit {
  ...
  this.rowData =  this.afs.collection(`boq/boq/generalareas`, ref => ref.orderBy("id", "asc").limit(40)).valueChanges()

  }

  Del(item){
    this.afs.doc(`boq/boq/generalareas/${item.rowData.id}`).delete().then(()=>{
      alert("Deleted")
    })
  ...
  onCellValueChanged(e){
    this.afs.doc(`boq/boq/generalareas/${e.data.id}`).update({
      code : e.data.code,
      name : e.data.name,
      price : e.data.price,
      unit : e.data.unit
  ...
  async delall(){
    ...
    this.afs.collection(`boq/boq/generalareas`).get().subscribe(value=>{
      ...
      value.docs.forEach(doc =>{
        this.afs.doc(`boq/boq/generalareas/${doc.ref.id}`).delete()
      })
